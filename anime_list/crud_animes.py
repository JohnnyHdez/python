...
class Crud:
    ...
    def animes_2025(self):
        cur = self.con.cursor()
        total = cur.execute("select id, name, season, chapters, year, last_chapter from emission where state='Finalizado' and last_chapter like '2025%' order by last_chapter, name")
        return total
    
    def total_vistos_2025(self):
        cursor = self.con.cursor()
        query = cursor.execute("SELECT COUNT(id) FROM emission WHERE last_chapter like '2025%' and state like 'Finalizado'")
        for data in query:
            total = data
        return total
...
class Ventana:
    ...
    def animes_vistos_2025(self):
        marco = tk.Toplevel()
        marco.geometry("1100x400")
        marco.minsize(width=900, height=300)
        marco.title("Animes vistos en 2025")
        ...
        result = crud.animes_2025()

        
        arial_12_bold = "Arial 12 normal"
        arial_14_bold = "Arial 14 bold"

        id_label = ttk.Label(master=panel, text="Id", font=(arial_14_bold), bootstyle=ttk.SUCCESS)
        name_label = ttk.Label(master=panel, text="Nombre", font=(arial_14_bold), bootstyle=ttk.SUCCESS)
        season_label = ttk.Label(master=panel, text="Temporadas", font=(arial_14_bold), bootstyle=ttk.SUCCESS)
        chapters_label = ttk.Label(master=panel, text="Capítulos", font=(arial_14_bold), bootstyle=ttk.SUCCESS)
        emited_label = ttk.Label(master=panel, text="Emitido", font=(arial_14_bold), bootstyle=ttk.SUCCESS)
        last_chapter_label = ttk.Label(master=panel, text="Último capítulo", font=(arial_14_bold), bootstyle=ttk.SUCCESS)
        total_animes_2023_label = ttk.Label(master=panel, text="Total: {}".format(crud.total_vistos_2025()[0]), font=(arial_14_bold), bootstyle=ttk.SUCCESS)
        ...
        for data in result:
            ttk.Label(master=panel, text=data[0], font=(arial_12_bold), foreground="cyan").grid(row=my_row, column=1)
            ttk.Label(master=panel, text=data[1], font=(arial_12_bold), foreground="cyan").grid(row=my_row, column=2)
            ttk.Label(master=panel, text=data[2], font=(arial_12_bold), foreground="cyan").grid(row=my_row, column=3)
            ttk.Label(master=panel, text=data[3], font=(arial_12_bold), foreground="cyan").grid(row=my_row, column=4)
            ttk.Label(master=panel, text=data[4], font=(arial_12_bold), foreground="cyan").grid(row=my_row, column=5)
            ttk.Label(master=panel, text=data[5], font=(arial_12_bold), foreground="cyan").grid(row=my_row, column=6)
    ...
    def formulario(self):
        ...
        #Animes seen in 2025        
        animes_seen = ttk.Button(master=self.panel, text="Animes vistos en 2025", bootstyle=ttk.SUCCESS)
        animes_seen.grid(row=1, column=14, ipadx=2)
        animes_seen.config(command=self.animes_vistos_2025)
